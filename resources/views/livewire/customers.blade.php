...
<div>
    ...
    @csrf
        <div class="flex justify-between gap-10 items-center mb-8">
            <h2 class="text-3xl font-semibold">{{ $isEditing ? 'Editar cliente' : 'Cadastrar cliente' }}</h2>
        ...
        <div class="grid grid-cols-2 gap-2">
            @if(!$isEditing)
                <div class="">
                    <label class="block text-black text-md font-bold">Email</label>
                    <input type="text" wire:model="email" placeholder="user@user.com" class="border hover:border-blue-400/50 outline-none focus:ring-2 focus:ring-blue-400 border-gray-500 rounded w-full p-2" required>
                    @error('email')
                ...
                <div class="">
                    <label class="block text-black text-md font-bold">Senha</label>
                    <input type="number" step="0.01" wire:model="password" placeholder="123456" class="hover:border-green-400/50 outline-none focus:ring-2 focus:ring-green-400 transition border border-gray-500 rounded w-full p-2" required>
                    @error('password')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="">
                <label class="block text-black text-md font-bold">Celular</label>
                <input type="number" step="0.01" wire:model="phone" placeholder="(00) 00000-0000" class="hover:border-green-400/50 outline-none focus:ring-2 focus:ring-green-400 transition border border-gray-500 rounded w-full p-2" required>
                @error('phone')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
